...
int get_student(int fd, int id, student_t *s)
{
    ...
    if (s->id == 0 || bytes_read != STUDENT_RECORD_SIZE) {
...
{
    student_t target;
    int rc = get_student(fd, id, &target);
...
int count_db_records(int fd)
{
    ...
    lseek(fd, 0, SEEK_SET);
    while (read(fd, &current, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
    ...
    return count >= 0 ? NO_ERROR : ERR_DB_FILE;
}
...
int print_db(int fd)
{
    ...
    while (1) {
        ssize_t bytes_read = read(fd, &current, STUDENT_RECORD_SIZE);
        ...
        if (memcmp(&current, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            ...
            float calculated_gpa = current.gpa / 100.0f;
            printf(STUDENT_PRINT_FMT_STRING, 
                   current.id, current.fname, current.lname, calculated_gpa);
            record_count++;
        }
    }
...
int compress_db(int fd)
{
    ...
    while (1) {
        ...
        if (memcmp(&current, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            if (write(temp_fd, &current, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
                printf(M_ERR_DB_WRITE);
                close(temp_fd);
                return ERR_DB_FILE;
            }
